...
def make_agent(path, obs_shape, n_actions, args):
    if "dqn" in args.folder.lower():
        return make_dqn(path, obs_shape, n_actions)
    else:
        return make_clean_rainbow(path, obs_shape, n_actions)
...
def evals(args):
    ...
    folders = [f for f in os.listdir("runs") if f.startswith(args.folder)]
    ...
    print(f"{args.folder} total steps avg: {total_steps.mean():,.1f} std: {total_steps.std():,.1f}")
