...
class Args:
    ...
    total_timesteps: int = 2_000_000#10_000_000
    """total timesteps of the experiments"""
    learning_rate: float = 1e-4#0.0000625
    """the learning rate of the optimizer"""
    num_envs: int = 1
    """the number of parallel game environments"""
    buffer_size: int = 500_000#1000_000
    """the replay memory buffer size"""
    gamma: float = 0.99
    """the discount factor gamma"""
    tau: float = 1.0
    """the target network update rate"""
    target_network_frequency: int = 2000#8000
    ...
    exploration_fraction: float = .2#0.10
    ...
    prioritized_replay_alpha: float = .6#0.5
    """alpha parameter for prioritized replay buffer"""
    prioritized_replay_beta: float = 0.4
    """beta parameter for prioritized replay buffer"""
    prioritized_replay_eps: float = 1e-5#1e-6
...
class NoisyDuelingDistributionalNetwork(nn.Module):
    def __init__(self, obs_shape, n_actions, n_atoms, v_min, v_max):
        super().__init__()
    ...
    def forward(self, x):
        h = self.feature_extractor(x.float())
...
if __name__ == "__main__":
    ...
    finally:  # ensure saving the model and closing the envs, even if there is an error during training or we hit a keyboard interrupt
        if args.save_model:
